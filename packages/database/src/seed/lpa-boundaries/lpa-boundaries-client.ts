...
const COUNTY_BOUNDARY_TIMEOUT_MS = 45_000;
 ...
 * planning.data.gov.uk entity API. See the
 * [API documentation](https://www.planning.data.gov.uk/docs) and the
 * [local planning authority dataset](https://www.planning.data.gov.uk/dataset/local-planning-authority)
 * for the response field definitions.
 *
 * @module LpaBoundariesClient
 */
export class LpaBoundariesClient {
	/**
	 * Fetches the county boundary for a given ONS 2025 county or unitary authority
	 * code (`CTYUA25CD`) from the ArcGIS Counties API. `CTYUA25NM` is the
	 * corresponding authority name.
	 */
	async getCountyBoundary(reference: string) {
		...
		const res = await this.fetchWithTimeout(url, COUNTY_BOUNDARY_TIMEOUT_MS);
		if (!res.ok) {
			let body;
			try {
				body = await res.text();
 ...
 *
 * Field definitions are documented in the
 * [API documentation](https://www.planning.data.gov.uk/docs) and the
 * [local planning authority dataset](https://www.planning.data.gov.uk/dataset/local-planning-authority).
 */
...
/**
 * The entity.json API response shape. See
 * https://www.planning.data.gov.uk/docs for API response and pagination details.
 */
...
/**
 * Guard function to check if the response is an ArcGIS error response.
 */
