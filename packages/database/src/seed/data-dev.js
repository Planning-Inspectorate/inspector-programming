...
function generateAppeals() {
	...
	// Create linked cases for randomly selected 50% of the appeals
	const linkedCases = [];

	// Calculate 50% of appeals to use as lead cases
	const leadCaseCount = Math.floor(appeals.length * 0.5);

	// Create an array of indices and shuffle it to randomly select lead cases
	...
	for (const leadIndex of selectedLeadIndices) {
		...
		for (let j = 0; j < 2; j++) {
			...
			linkedCases.push({
				...
				siteAddressLongitude: location.siteAddressLongitude,
				caseReference: reference,
				leadCaseReference: leadCaseReference,
				caseCreatedDate: created,
				caseValidDate: valid,
				finalCommentsDueDate: finalCommentsDue,
				Specialisms: generateCaseSpecialisms(),
