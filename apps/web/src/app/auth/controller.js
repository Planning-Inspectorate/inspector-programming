 ...
 * @returns {import('express').RequestHandler<?, ?, ?, { redirect_to?: string | string[] }>}
 */
export function buildStartMsalAuthentication(authService) {
	return async (request, response) => {
		...
		if (redirectTo && !isValidRedirectUri(redirectTo)) {
...
function isValidRedirectUri(uri) {
	...
	const pathPaths = uri.split('/');
	// prevent any path traversal
	if (pathPaths.some((part) => part === '..' || part === '.')) {
		return false;
	}
	return true;
}
 ...
 * @param {import('./auth-service.js').AuthService} authService
 * @returns {import('express').RequestHandler<?, ?, ?, { code?: string }>}
 */
export function buildCompleteMsalAuthentication(logger, authService) {
	return async (request, response) => {
 ...
 */
export function buildHandleSignout(logger, logoutUri, authService) {
