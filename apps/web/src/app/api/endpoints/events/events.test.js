...
describe('events', () => {
	describe('GET /events', () => {
		...
		test('excludes events with isCancelled flag set to true', async (ctx) => {
			...
			mock.method(mockService.apiService.entraClient, 'listAllGroupMembers', async () => [
				{
					...
					givenName: 'test',
					surname: 'user',
					mail: 'test-user@planninginspectorate.gov.uk'
				}
			]);
			...
			assert.strictEqual(events[0].id, 'evt-active');
		});

		test('excludes events with CANCELLED: prefix in subject', async (ctx) => {
			mockService.entraConfig.groupIds.inspectorGroups = 'groupA';
			mockService.entraConfig.calendarEventsDayRange = 3;
			mock.method(mockService.apiService.entraClient, 'listAllGroupMembers', async () => [
				{
		...
		test('includes events without isCancelled field', async (ctx) => {
			mockService.entraConfig.groupIds.inspectorGroups = 'groupA';
			mockService.entraConfig.calendarEventsDayRange = 3;
			mock.method(mockService.apiService.entraClient, 'listAllGroupMembers', async () => [
				{
			...
			mock.method(mockService.apiService.entraClient, 'listAllUserCalendarEvents', async () => [
				{
					id: 'evt-no-flag',
					subject: 'Meeting without cancel flag',
					start: { dateTime: dates.twoDaysAgo.toISOString() },
					end: { dateTime: dates.oneDayAgo.toISOString() },
					isAllDay: false,
					showAs: 'busy',
			...
			const events = await res.json();
			assert.strictEqual(events.length, 1);
			assert.strictEqual(events[0].id, 'evt-no-flag');
		});

		test('excludes malformed events and logs error', async (ctx) => {
			mockService.entraConfig.groupIds.inspectorGroups = 'groupA';
			mockService.entraConfig.calendarEventsDayRange = 3;
			mock.method(mockService.apiService.entraClient, 'listAllGroupMembers', async () => [
				{
			...
			const events = await res.json();
			assert.strictEqual(events.length, 1);
			assert.strictEqual(events[0].id, 'evt-valid');
			assert.strictEqual(mockService.apiService.logger.error.mock.callCount(), 1);
		});
	});
