...
class ElementError extends GOVUKFrontendError {
	constructor(t) {
		...
		if ('object' == typeof t) {
			...
			((e = n),
				(e += s ? ` is not of type ${null != o ? o : 'HTMLElement'}` : ' not found'),
...
class InitError extends GOVUKFrontendError {
	constructor(t) {
		(super('string' == typeof t ? t : formatErrorMessage(t, 'Root element (`$root`) already initialised')),
			(this.name = 'InitError'));
	}
}
class Component {
	...
	constructor(e, i) {
		...
		if (!isObject(n.defaults))
			throw new ConfigError(
				formatErrorMessage(n, 'Config passed as parameter into constructor but no defaults defined')
			);
		const s = (function (Component, t) {
			if (!isObject(Component.schema))
				throw new ConfigError(
					formatErrorMessage(Component, 'Config passed as parameter into constructor but no schema defined')
				);
			const e = {},
			...
			for (const n of i) {
				...
				(o in t && (e[o] = normaliseString(t[o], s)),
					'object' === (null == s ? void 0 : s.type) && (e[o] = extractConfigByNamespace(Component.schema, t, i)));
			}
			return e;
...
function normaliseString(t, e) {
	...
	switch (
		(s || (['true', 'false'].includes(i) && (s = 'boolean'), i.length > 0 && isFinite(Number(i)) && (s = 'number')), s)
	) {
		case 'boolean':
			n = 'true' === i;
			break;
...
function extractConfigByNamespace(t, e, i) {
	...
	for (const [o, r] of Object.entries(e)) {
		...
		for (const [n, s] of e.entries())
			isObject(t) &&
				(n < e.length - 1 ? (isObject(t[s]) || (t[s] = {}), (t = t[s])) : o !== i && (t[s] = normaliseString(r)));
	}
	return s[i];
}
class I18n {
	constructor(t = {}, e = {}) {
		...
		((this.translations = void 0),
			(this.locale = void 0),
			(this.translations = t),
			(this.locale = null != (i = e.locale) ? i : document.documentElement.lang || 'en'));
	}
	t(t, e) {
...
class Accordion extends ConfigurableComponent {
	constructor(t, e = {}) {
		...
		if (!i.length)
			throw new ElementError({ component: Accordion, identifier: `Sections (\`<div class="${this.sectionClass}">\`)` });
		((this.$sections = i),
			this.initControls(),
...
class CharacterCount extends ConfigurableComponent {
	...
	constructor(t, e = {}) {
		...
		((this.i18n = new I18n(this.config.i18n, { locale: closestAttributeValue(this.$root, 'lang') })),
			(this.maxLength = null != (i = null != (n = this.config.maxwords) ? n : this.config.maxlength) ? i : 1 / 0),
			(this.$textarea = s));
		const r = `${this.$textarea.id}-info`,
			a = document.getElementById(r);
		if (!a)
			throw new ElementError({ component: CharacterCount, element: a, identifier: `Count message (\`id="${r}"\`)` });
		((this.$errorMessage = this.$root.querySelector('.govuk-error-message')),
			`${a.textContent}`.match(/^\s*$/) &&
				(a.textContent = this.i18n.t('textareaDescription', { count: this.maxLength })),
...
class Checkboxes extends Component {
	constructor(t) {
		...
		if (!e.length)
			throw new ElementError({ component: Checkboxes, identifier: 'Form inputs (`<input type="checkbox">`)' });
		((this.$inputs = e),
			this.$inputs.forEach((t) => {
				const e = t.getAttribute('data-aria-controls');
				if (e) {
					if (!document.getElementById(e))
						throw new ElementError({ component: Checkboxes, identifier: `Conditional reveal (\`id="${e}"\`)` });
...
class FileUpload extends ConfigurableComponent {
	constructor(t, e = {}) {
		...
		if (null === i)
			throw new ElementError({ component: FileUpload, identifier: 'File inputs (`<input type="file">`)' });
		if ('file' !== i.type)
			throw new ElementError(
				formatErrorMessage(FileUpload, 'File input (`<input type="file">`) attribute (`type`) is not `file`')
			);
		if (((this.$input = i), this.$input.setAttribute('hidden', 'true'), !this.$input.id))
			throw new ElementError({
		...
		const c = document.createElement('span');
		((c.className = 'govuk-button govuk-button--secondary govuk-file-upload-button__pseudo-button'),
	...
	onChange() {
		...
		0 === t
			...
			: ((this.$status.innerText =
					1 === t ? this.$input.files[0].name : this.i18n.t('multipleFilesChosen', { count: t })),
				this.$button.classList.remove('govuk-file-upload-button--empty'));
	}
	findLabel() {
		const t = document.querySelector(`label[for="${this.$input.id}"]`);
		if (!t)
			throw new ElementError({ component: FileUpload, identifier: `Field label (\`<label for=${this.$input.id}>\`)` });
		return t;
	}
	onClick() {
		this.$input.click();
...
((FileUpload.moduleName = 'govuk-file-upload'),
	...
	(FileUpload.schema = Object.freeze({ properties: { i18n: { type: 'object' } } })));
	...
	constructor(t) {
		...
		((this.$inputs = e),
			this.$inputs.forEach((t) => {
				...
				if (e) {
					if (!document.getElementById(e))
						throw new ElementError({ component: Radios, identifier: `Conditional reveal (\`id="${e}"\`)` });
					(t.setAttribute('aria-controls', e), t.removeAttribute('data-aria-controls'));
				}
			}),
			window.addEventListener('pageshow', () => this.syncAllConditionalReveals()),
...
class Tabs extends Component {
	constructor(t) {
		...
		if (!n.length)
			throw new ElementError({ component: Tabs, identifier: 'List items (`<li class="govuk-tabs__list-item">`)' });
	...
	onTabClick(t) {
		...
		e &&
			...
			(t.preventDefault(), this.hideTab(e), this.showTab(i), this.createHistoryEntry(i));
	}
...
function initAll(t) {
	...
	if (((t = void 0 !== t ? t : {}), !isSupported()))
		return void (t.onError ? t.onError(new SupportError(), { config: t }) : console.log(new SupportError()));
	const i = [
...
function createAll(Component, t, e) {
	...
	return isSupported()
		? Array.from(o)
				...
				.filter(Boolean)
		: (i ? i(new SupportError(), { component: Component, config: t }) : console.log(new SupportError()), []);
}
