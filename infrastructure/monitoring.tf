...
resource "azurerm_monitor_action_group" "scheduling_tech" {
  name                = "pins-ag-scheduling-tech-${var.environment}"
  resource_group_name = azurerm_resource_group.primary.name
  short_name          = "SchedulDev" # needs to be under 12 characters
  tags                = local.tags
...
resource "azurerm_monitor_action_group" "scheduling_service_manager" {
  name                = "pins-ag-scheduling-service-manager-${var.environment}"
  resource_group_name = azurerm_resource_group.primary.name
  short_name          = "SchedulDev" # needs to be under 12 characters
  tags                = local.tags
...
resource "azurerm_application_insights_standard_web_test" "web" {
  ...
  request {
    ...
    url = "https://${var.web_domains.web}/health"
  }
  validation_rules {
