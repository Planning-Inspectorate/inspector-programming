...
data "azurerm_servicebus_topic" "appeal_event" {
  name         = var.service_bus_config.topics.appeal_event
  namespace_id = data.azurerm_servicebus_namespace.appeals.id
}
...
resource "azurerm_servicebus_subscription" "appeal_event" {
  name                                 = "appeal-event-scheduling-sub"
  topic_id                             = data.azurerm_servicebus_topic.appeal_event.id
  max_delivery_count                   = 1
  dead_lettering_on_message_expiration = true
  default_message_ttl                  = var.service_bus_config.ttl
}
...
resource "azurerm_role_assignment" "appeal_event_receiver" {
  scope                = azurerm_servicebus_subscription.appeal_event.id
  role_definition_name = "Azure Service Bus Data Receiver"
  principal_id         = module.function_integration.principal_id
}
...
# peer to Manage appeals VNET for integration
